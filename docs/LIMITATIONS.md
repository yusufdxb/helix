...
The Session 8 closed-loop demonstration also depends on the R1 schema fix on branch `fix/r1-anomaly-schema-mismatch @ e821b47`. Reproducing the loop from `main` will silently produce 0 hints because the on-`main` R1 rule reads keys the live emitter does not write. This will be resolved when the fix lands on `main`.

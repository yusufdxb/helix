# Ecosystem comparison: what HELIX adds to ROS 2

This document answers one question: given that `diagnostic_updater`,
...
adding? Where the answer is "nothing, this duplicates an existing
package," it says so.

The comparison evaluates this claim: **ROS 2 has no central error/fault manager and no
...
to *gated actuation* as one system with one safety envelope. The value of
HELIX depends on whether that integration gap holds up.
...
| `ros-safety/software_watchdogs` | `SimpleWatchdog` (liveliness-only) and `WindowedWatchdog` (liveliness + deadline, tolerates N lease violations) transition a lifecycle node to `Inactive` on a DDS LIVELINESS/DEADLINE violation. Canonical, WG-blessed mechanism for detecting a dead or wedged publisher without extra topic traffic. | `HeartbeatMonitor`'s original design (a bespoke `std_msgs/String` name published at 10 Hz, counted against a wall-clock timeout) is a less standard reimplementation of what `software_watchdogs` already does at the DDS layer. `helix_core/liveliness_beacon.py` and `helix_core/liveliness_monitor.py` implement the `SimpleWatchdog` half of that mechanism directly against rclpy QoS rather than depending on the external package. HELIX duplicates this mechanism rather than wrapping it. | `software_watchdogs` transitions *the node itself* to Inactive on its own lease violation: it is a per-node self-defense primitive. It has no concept of a fault taxonomy (CRASH vs ANOMALY vs LOG_PATTERN), no rule engine mapping fault type to a recovery action, and no downstream actuation gate. A `WindowedWatchdog` firing tells you a node died; it does not tell a different, still-alive node ("recovery") what to do about it, and it does not stop `/cmd_vel`. |
...
## Where HELIX duplicates existing work
...
- **`/helix/node_health` duplicates `diagnostic_aggregator`.** There is no
  ...
  grouping. The current implementation retains this duplication and
...
- **The original heartbeat mechanism duplicated, worse, what
  ...
  motivates the liveliness implementation described below. It is not a new
  detection *capability*, only a
...
## Liveliness design context

The heartbeat fix in `helix_core/heartbeat.py` made `HeartbeatMonitor`'s existing
...
The `helix_core/liveliness_beacon.py` and
...
nodes or the bringup launch files.

### Recommendation: do not switch the default yet

**Keep `HeartbeatMonitor` (the String-topic heartbeat) as the default.
Do not switch to DDS liveliness as the default detection path yet.** Tradeoffs,
in order of weight:
...
4. **Only `SimpleWatchdog`-equivalent behavior is implemented.** One lease
   ...
   `liveliness_monitor.py` as an explicit non-goal). `HeartbeatMonitor`'s
   `miss_threshold` already gives that
...
5. **It is unwired and unproven at the fleet level.** The only evidence
   ...
   detection path needs before it becomes the default. The heartbeat path has
   that evidence through a real process kill against `helix_context_buffer`.

The recommended path is to keep both. Ship DDS liveliness as an

...
- ~~HELIX resource overhead on Jetson, projected from PC measurement × algorithmic scaling factor~~ → **MEASURED (Sessions 5-7, see §3, §15, §17)**
...
- Performance under Jetson thermal throttling (Session 7 peak 52.9°C, 32°C below the 85°C throttle, so not yet stressed)
...
- CRASH path: published `std_msgs/String("pc_test_node")` on `/helix/heartbeat` 15x at 10 Hz, then stopped. Detector emitted a single CRASH FaultEvent at 1.60 s (matches `check_interval_sec=0.5 x miss_threshold=3 = 1.5 s`); `_crashed_nodes` dedup correctly suppressed repeat emissions.
- ANOMALY path: published Float64MultiArray on `/helix/metrics` with 65 baseline samples at 10.0 +/- 0.1 followed by 5x spikes at 50.0. Detector emitted 3 ANOMALY FaultEvents (zscore 5.38 / 4.36 / 3.74 as the rolling window absorbed the outliers).

**Live GO2 anomaly detection:** during a 20-min stability run (1201 s, 120 samples) with concurrent load from two other projects on the same Jetson, the adapter plus anomaly_detector emitted **60 real ANOMALY FaultEvents** across 6 distinct metric labels:
...
Session 4 (2026-04-14) crashed at 866 s under the same concurrent load. Session 6 ran through the same concurrency pattern and passed cleanly: all 6 nodes `active [3]` for every one of the 120 samples, 0 transitions, 0 deaths.

| Metric | First sample | Last sample | Peak |
|---|---|---|---|
| Total RSS (6 nodes) | 250.1 MB | 263.5 MB | 263.5 MB |
| Total CPU (sum) | n/a | n/a | 56.7% |
| Jetson load1 | 8.53 | 4.40 | 12.71 |
| Thermal max | 57.5°C | 56.9°C | 58.2°C |

**The +13 MB over 20 min (+0.7 MB/min) was left open at Phase 2**, either a leak or warm-up, and was resolved by Session 7 (§15) as pre-plateau warm-up.
...
- 20-min stability under concurrent load, past Session 4's 866 s failure point
...
**Total RSS grew 257.16 MB to 281.77 MB and plateaued**. Growth rate decelerated 7x between halves (0.717 MB/min for minutes 0-30, 0.103 MB/min for 30-60), and the total RSS was already flat at 281.77 MB from sample 226 (t ~ 38 min) through sample 359 (t ~ 60 min).

Per-node RSS and CPU over the hour. This table is the Python baseline the C++ port is scored against in `docs/cpp_parity_summary.md`:

| Node | RSS t=0 (MB) | RSS t=60 min (MB) | Delta (MB) | CPU mean (%) | CPU peak (%) |
|---|---:|---:|---:|---:|---:|
...
These deltas refute the Phase 2 hypothesis that the trend was a `topic_rate_monitor` callback leak: `topic_rate_monitor` had the *smallest* RSS growth (+2.48 MB) despite burning ~40x more CPU than any other node. Growth was spread evenly across all 6 processes (+2.5 to 5.1 MB each), consistent with ordinary Python heap warm-up.
...
| Benchmark | 2026-04-06 | 2026-04-17 | Delta |
|---|---:|---:|---|
...
| Node | CPU (%) with IMU (Phase 3 1-hr) | CPU (%) no-IMU | Delta |
|---|---:|---:|---:|
...
The 250 Hz `/utlidar/imu` rate-monitor callback alone accounts for ~94% of `topic_rate_monitor`'s CPU and ~88% of the whole 6-node adapter stack. **For deployments where `/utlidar/imu` rate monitoring is not required, drop it from `helix_topic_rate_monitor.topics`: adapter CPU drops from ~48% of one Jetson core to under 6%, with no effect on the fault-detection paths for the other 5 topics.**

### Updated "Observed on Hardware" additions (Session 7)

- 1-hour RSS plateau at 282 MB, no leak, which refutes the Phase 2 attribution
- Per-node Python RSS and CPU baseline for all 6 lifecycle nodes (§15), the reference the C++ port is scored against
- Session 1 benchmark numbers reproduce within +/-2% across 11 days
- Adapter CPU is configurable from 48% down to 6% via a single `topics` edit

#!/usr/bin/env bash
# Task 3 v2 — same as v1 but launches inside a tmux session so the process tree
# survives shell/SSH lifecycle quirks. Each node also gets its own stderr log
# via stdbuf so any silent crash leaves a traceback.
...
# Launch each node in its own detached tmux window with line-buffered stderr capture
tmux kill-session -t helix_t3 2>/dev/null
tmux new-session -d -s helix_t3 -n hb \
  "stdbuf -oL -eL ros2 run helix_core helix_heartbeat_monitor \
     --ros-args -r __node:=helix_heartbeat_monitor --params-file $PARAMS \
     2>&1 | tee $OUT/hb.log"
tmux new-window -t helix_t3 -n ad \
  "stdbuf -oL -eL ros2 run helix_core helix_anomaly_detector \
     --ros-args -r __node:=helix_anomaly_detector --params-file $PARAMS \
     2>&1 | tee $OUT/ad.log"
tmux new-window -t helix_t3 -n lp \
  "stdbuf -oL -eL ros2 run helix_core helix_log_parser \
...
# Background watchers, all detached via setsid+nohup
( setsid nohup tegrastats --interval "$TEGRA_PERIOD_MS" > "$OUT/tegrastats.log" 2>&1 ) &
TEGRA_PID=$!
...
export RMW_IMPLEMENTATION=rmw_cyclonedds_cpp
export CYCLONEDDS_URI=file:///home/unitree/unitree_ros2/cyclonedds_ws/src/cyclonedds.xml
ros2 topic echo --no-arr --no-str /helix/faults helix_msgs/msg/FaultEvent" > "$OUT/faults_echo.log" 2>&1 ) &
FAULTS_PID=$!
...
sample_one() {
  ...
  # Lifecycle queries can hang — kill cleanly via timeout
